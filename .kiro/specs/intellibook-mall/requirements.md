...
IntelliBook-Mall 是一个基于智能推荐的网上电子书商城系统，融合了 Z-library 的庞大书库管理模式和传统电商的购买功能。系统采用前后端分离架构，后端基于 Spring Boot 3.x，前端使用 Vue.js，数据库采用 MySQL。

本项目重度参考 newbee-mall 开源项目的后端架构，但针对电子书业务进行了大规模改造，包括电子书元数据管理、在线阅读、智能推荐等核心功能。
...
1. WHEN 用户首次注册，THE IntelliBook系统 SHALL 引导用户选择至少3个感兴趣的分类
2. THE IntelliBook系统 SHALL 允许用户在个人中心添加或删除兴趣标签
3. THE IntelliBook系统 SHALL 基于用户选择的兴趣标签生成个性化推荐
4. WHEN 用户未设置兴趣标签，THE IntelliBook系统 SHALL 展示热门电子书和编辑推荐
5. THE IntelliBook系统 SHALL 支持用户选择多个兴趣分类（建议3-10个）
6. THE IntelliBook系统 SHALL 在推荐算法中优先考虑用户兴趣标签

**对比 newbee-mall**:
- 新增：完全新增的功能模块，newbee-mall 无此功能
- 需要：用户兴趣表
- 目的：解决推荐系统的冷启动问题，提升新用户体验
...
| 技术组件 | newbee-mall | IntelliBook-Mall (毕业设计版) | 说明 |
|---------|-------------|------------------------------|------|
| 后端框架 | Spring Boot 3.1.11 | Spring Boot 3.1.11+ | 保持稳定 |
| Java 版本 | 17 | 17+ | 保持稳定 |
| ORM 框架 | MyBatis | MyBatis 3.5.x | 保持一致 |
| 数据库 | MySQL | **SQLite 3** | **简化为嵌入式数据库** |
| 接口文档 | SpringDoc (OpenAPI) | SpringDoc 2.x | 保持一致 |
| 前端框架 | Vue.js | Vue 3.4.x+ | 保持稳定 |
| 前端UI库 | Element Plus | Element Plus 2.x | 保持一致 |
| 文件存储 | 本地存储 | 本地文件系统 | 简化方案 |
| 推荐算法 | 无 | 简单推荐（可选） | 简化实现 |
| 阅读器 | 无 | 文件下载（可选在线阅读） | 简化功能 |
| 缓存 | 无 | 无（简化） | 去除 Redis |

**毕业设计版本说明**：
- **数据库简化**：MySQL → SQLite，零配置、便携、适合演示
- **表结构简化**：15+ 张表 → 8 张核心表
- **功能精简**：保留核心功能（85%+），删除复杂的推荐系统、阅读进度等
- **开发效率**：提升 50%，适合毕业设计时间要求
- **答辩友好**：导师可以直接运行项目，无需配置数据库
- **技术展示**：仍然可以展示完整的技术栈和系统设计能力
